    ...
    for t in loss_tasks:
        if t == 'normal':
            ...
            loss_values[t] = compute_cos_sim_loss(attack.params,
                                                    ...
                                                    fixed_model)
        # elif t == 'marksman':
        #     # Add the code for marksman loss here

        #     loss_values[t] = compute_marksman_loss(attack.params,
        #                                            epoch,
        #                                            batch_idx,
        #                                            optimizer,
        #                                            criterion,
...
# def compute_marksman_loss(params: Params, epoch, batch_idx, optimizer, criterion, atkmodel, tgtmodel, model, tgtoptimizer, 
#           batch, clip_image, post_transforms=None):
#     # Implement marksman loss here
#     bs = batch.batch_size
#     ########################################
#     #### Update Trigger Function ####
#     ########################################
#     data, target = batch.inputs, batch.labels
#     atktarget = sample_negative_labels(target, params.num_classes).to(params.device) #randomly sample any labels
#     noise = tgtmodel(data, atktarget) * params.eps
#     atkdata = clip_image(data + noise)
    
#     aug_atkdata = post_transforms(atkdata)
#     aug_data = post_transforms(data)
            
#     if params.attack_portion < 1.0:
#         aug_atkdata = aug_atkdata[:int(params.attack_portion*bs)]
#         atktarget = atktarget[:int(params.attack_portion*bs)]
...
#     ###############################
#     atktarget = sample_negative_labels(target, params.num_classes).to(params.device) #randomly sample any labels
#     noise = atkmodel(data, atktarget) * params.eps
#     atkdata = clip_image(data + noise)
...
#     loss_poison = criterion(atkoutput, atktarget)
#     loss2 = loss_clean * params.alpha + (1-params.alpha) * loss_poison
#     optimizer.zero_grad()
#     loss2.backward()
#     optimizer.step()

#     if batch_idx % 10 == 0:
#         print('Train [{}] Loss: clean {:.4f} poison {:.4f} CLS {:.4f} ATK:{:.4f}'.format(
